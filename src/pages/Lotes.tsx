...
import { Plus, Package, Users, TrendingUp, Loader2, Trash2, Eye, Utensils, DollarSign } from 'lucide-react';
...
export default function Lotes() {
  ...
  // Fetch dietas ativas para o select
    ...
    queryFn: async () => {
      ...
      const lotesWithStats = await Promise.all(
        lotesData.map(async (lote) => {
          ...
          // Calcular custo diário da dieta
          let custoDietaDia = 0;
          if (lote.dietas) {
            const dieta = lote.dietas as any;
            custoDietaDia = Number(dieta.consumo_diario_kg || 0) * Number(dieta.custo_por_kg || 0);
          }
          ...
          return { 
            ...
            custoDietaDia,
            custoDietaMes: custoDietaDia * 30,
            custoTotalDia: custoDietaDia * totalAnimais,
            custoTotalMes: custoDietaDia * totalAnimais * 30,
          };
        })
      );
  ...
  // Calcular preview da dieta selecionada
  ...
  const createLoteMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['lotes-detalhados'] });
    ...
    <AppLayout>
      <div className="container py-6">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <Select value={dietaId} onValueChange={setDietaId}>
                    <SelectTrigger><SelectValue placeholder="Selecione uma dieta..." /></SelectTrigger>
                    <SelectContent>
                      {dietas?.map((dieta) => (
                        <SelectItem key={dieta.id} value={dieta.id}>
                          {dieta.nome} {dieta.tipo && `(${dieta.tipo})`}
                        </SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                  {dietaSelecionada && (
                    <p className="text-sm text-muted-foreground flex items-center gap-1">
                      ...
                      Esta dieta custa <span className="font-medium text-primary">{formatCurrency(previewCustoDiario)}/dia</span> por animal
                    </p>
                  )}
                </div>
                <div className="space-y-2">
        ...
        {isLoading ? (
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {lotes.map((lote) => (
              <Card key={lote.id} className="transition-shadow hover:shadow-md cursor-pointer" onClick={() => navigate(`/lotes/${lote.id}`)}>
                <CardHeader className="pb-2">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Button variant="ghost" size="icon" className="h-8 w-8 text-destructive hover:bg-destructive/10" onClick={(e) => { e.stopPropagation(); setDeleteModal({ open: true, lote, animais: lote.totalAnimais }); }}>
                  ...
                  <div className="space-y-2">
                    ...
                    {(lote.dietas as any) && (
                      <div className="flex items-center gap-2 text-sm">
                        ...
                        <span>Dieta: {(lote.dietas as any).nome}</span>
                      </div>
                    )}
                    {lote.custoTotalDia > 0 && lote.totalAnimais > 0 && (
                      <div className="flex items-center gap-2 text-sm text-green-600">
                        <DollarSign className="h-4 w-4" />
                        <span>{formatCurrency(lote.custoTotalDia)}/dia</span>
                      </div>
                    )}
                    {lote.capacidade && (
